...
| Step size `h` | **One global `h` = 1/512** (0.001953125) for every sigma, for now. The two-tier scheme below is measured and correct, but it is config surface bought before anything has run end to end — **deferred, not overturned**. Cost: 209,280 velocity evaluations across the four-sigma sweep against 72,192 for two tiers — 2.9x, or ~2 h on 8 GPUs at the measured 280 ms/eval — and compute is not the binding constraint here, Monte-Carlo error is. The two-tier reasoning is kept in the accuracy table below because it is the evidence for revisiting: `0.01 -> 1/512`, everything else `-> 1/64`; 1/256 everywhere leaves sigma=0.01 only 17x margin. Revisit with the phase-8 halve-h study. |
